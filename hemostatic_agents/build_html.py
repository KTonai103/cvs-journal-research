#!/usr/bin/env python3
"""Build the search-enabled, sidebar-navigated hemostatic-agents review HTML.
...
Usage:
  python3 hemostatic_agents/build_html.py            # defaults to the integrated review
  python3 hemostatic_agents/build_html.py in.md out.html
"""
import os
import re
import sys
...
DESCRIPTION = ("心臓外科の止血剤・シーラント（ボルヒール／サージフロー／フロシール／ハイドロフィット／"
               "バイオグルー／タコシール等）の作用機序・メリット/デメリット・日本での価格と保険償還、"
               "および Hydrofit の仮性動脈瘤・感染懸念の文献的検証")
...
def main():
    if len(sys.argv) == 3:
        ...
        md = os.path.join(BASE, "md", "hemostatic_agents_review.md")
        out = os.path.join(ROOT, "output", "hemostatic_agents_review.html")
        build(md, out)
