...
DESCRIPTION = (
    "Commando Procedure（Double Valve Double Patch Repair / Fibrous Skeleton Reconstruction）統合レビュー — "
    "大動脈弁・僧帽弁間線維体（intervalvular fibrosa / aortomitral curtain）が破壊された浸潤性感染性心内膜炎に対する "
    "Commando・hemi-Commando・Root-Commando の適応・手技ステップ・Pitfalls・施設別成績を全32論文から統合し、"
    "原典PDFの術中写真・手技図・推奨表54点を該当箇所に挿入した術式リファレンス")
...
def main():
    if len(sys.argv) == 3:
        ...
        md = os.path.join(BASE, "md", "Commando_Procedure_IntegratedReview_2026.md")
        out = os.path.join(ROOT, "output", "commando_procedure_review.html")
        build(md, out)
