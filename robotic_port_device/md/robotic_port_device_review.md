...
| **③ 電メ切開による熱傷創の解決** | **単独の売り文句にはできないが、心臓外科側には支持する専門家記述がある** | 腹部の皮膚切開線に関する限り Cochrane・複数のメタ解析はいずれも「電メは同等ないし優位」。**一方でロボット心臓外科の専門家記述は逆で、「Cauterization should be performed intrathoracic to **avoid delayed wound healing, infection and to ensure proper cosmesis**」と明示している（Patel/Macoskey 2022）**。**「ポート孔を全層焼き広げる」操作の比較研究は世界的に皆無＝未検証** |
...
| 要素 | 判定 |
|---|---|
| バルーンで体壁内側に引っ掛けて逸脱防止＋圧迫止血 | **新規性なし**（八光 E・ZバルーンⅡ、Applied Medical Kii、[US 8,142,467](https://www.freepatentsonline.com/8142467.html)、Balkhy の臨床実装） |
| 止血材をアクセスデバイス外周に事前装着 | **製品ゼロ・登録クレームゼロ＝白地**。ただし WO 2019/122487（失効）と [US 7,018,392](https://www.freepatentsonline.com/7018392.html)（満了）が公開文献として存在 |
| 胸腔／肋間専用 | **バルーン式の胸腔専用ポートは製品として不存在**。ただし機械式の肋間アンカーは [US 5,776,110](https://www.freepatentsonline.com/5776110.html)（1996）が公知 |
...
### 1.3.1 ロボット心臓外科の専門家自身が「ポート孔は制御困難な出血源」と書いている〔一次確認済〕

Patel NC, Macoskey AR. "Complications and their management in robotic mitral valve surgery from the surgical assistant's perspective." Ann Cardiothorac Surg 2022;11(5):510-24. [PMID 36237594](https://pubmed.ncbi.nlm.nih.gov/36237594/) / PMC9551371（OA、全文で逐語確認）：

- 「**Port sites are common sources of bleeding and oftentimes difficult to control.** The surgeon must be persistent and confirm hemostasis with direct visualization before closure.」
- 「At our institution, **if there is persistent port site bleeding despite cautery and manual compression, the port site incision is extended.**」← 現行の対処法の限界。**焼灼と用手圧迫で止まらなければ創を延長して直視下に処置するしかない**
- 「If **intercostal bleeding** is encountered, hemostasis must be achieved before proceeding onto the next step. Hemostasis is achieved with cautery, surgical clip, oversewing stitch or manual compression.」

→ **構想の問題設定は、ロボット心臓外科の専門家自身の記述で裏づけられている。** しかも「創を延長する」という解決法しか無い、と書かれている点が、専用デバイスの余地を示している。
...
| 単肺換気＋CO2 送気 | **点検に使える時間が構造的に足りない。** 逐語：「Single lung ventilation shortly after separating from CPB may cause hemodynamic compromise, so **intrathoracic assessment and hemostasis must be efficient**」＝ CPB 離脱直後の片肺換気は血行動態を崩すため、胸腔内の点検・止血は「手早く」やらざるを得ない | Patel NC, Macoskey AR. Ann Cardiothorac Surg 2022;11(5):510-24, [PMID 36237594](https://pubmed.ncbi.nlm.nih.gov/36237594/) / PMC9551371〔一次確認済〕 |
...
### 2.2 特許 — 障壁になるのは [US 8,142,467](https://www.freepatentsonline.com/8142467.html) 系だけ〔一次確認済〕
...
1. **FTO の主障壁は Intuitive ではなく Applied Medical、しかも [US 8,142,467](https://www.freepatentsonline.com/8142467.html) 系1本に絞られる。** [US 8,287,503](https://www.freepatentsonline.com/8287503.html) は止血を目的としていないので射程外
2. **「バルーン＋止血材」の組合せ自体は 2001年出願・2006年登録の [US 7,018,392](https://www.freepatentsonline.com/7018392.html) が既にクレームしている**（満了済）。新規性（体壁ポートでない）は壊れないが、**進歩性の壁になる**
3. **「肋間の内側に引っ掛けて抜けを防ぐ」着想も 1996年から公知**（[US 5,776,110](https://www.freepatentsonline.com/5776110.html)、機械式・満了済）
...
### 4.3 ただし心臓外科側には「電メ警告」の専門家記述が実在する〔一次確認済〕

§4.2 はすべて**腹部・鼠径部の皮膚切開線**の話である。**ロボット心臓外科の側には、正反対の記述がある。**

Patel NC, Macoskey AR（Ann Cardiothorac Surg 2022, [PMID 36237594](https://pubmed.ncbi.nlm.nih.gov/36237594/) / PMC9551371）のポート作成の項、逐語：

> 「Hemostasis is achieved with cautery, surgical clip, oversewing stitch or manual compression. **Cauterization should be performed intrathoracic to avoid delayed wound healing, infection and to ensure proper cosmesis.**」

**＝ 焼灼は「創傷治癒の遅延・感染・整容性の悪化」を避けるため胸腔内側で行え、とロボット心臓外科の専門家が明示的に指示している。** これはユーザーの前提③（電メで開けると治りが悪い）と同じ懸念を、心臓外科の現場が共有していることを示す唯一の一次記述である。

→ **したがって前提③は「否定された」のではなく、「一般外科の皮膚切開線のデータでは支持されないが、ロボット心臓外科の専門家意見としては存在する」が正確な整理である。** 提案書では「Patel/Macoskey がこう書いている。しかし比較研究は無い。だから我々が測る」という立て方が最も強い。

### 4.4 それでも構想を救う余地はある〔要一次確認〕
...
### 最優先（これを先にやらないと開発が空振りする）

| # | 内容 |
|---|---|
| **1** | **自施設＋多施設で「出血源内訳」を取る。** ロボット/MICS 心臓手術の再開胸例について、Pojar 2019 と同じフォーマット（肋間動脈／ポート創／カニュレーション部位／心内縫合線／不明）で記録する。**ロボット 8mm ポートに限った内訳は世界に一件も存在しない**ため、この記述研究そのものが英文原著になり、同時にデバイスの unmet need の唯一の定量的根拠になる |
| **2** | **開発ターゲットを working port に定める（決定事項として）。** 第3/4肋間・1.5–4cm 切開・ソフト組織リトラクタ使用の working port は、⑴最も径が大きく肋間剥離が大きく最も出血する、⑵**ロボットアームにドッキングしないため Intuitive の IP が原理的にかからない**、⑶ソフト組織リトラクタという既存製品の置換／改良として市場導入経路が明確。8mm アームポートは第二フェーズとし、**純正カニューラの管部に被せる体壁側スリーブ**（ConMed AirSeal dV の前例）に限定する。マウント／滅菌アダプタ界面に噛む形状は [US 11,197,729](https://www.freepatentsonline.com/11197729.html) に抵触しうるため**設計から除外** |
| **3** | **弁理士による正式な FTO ／先行技術調査を発注。** 指定すべき対象：⒜ US 2025/0025249 A1（出願18/777,746）の file wrapper、⒝ Applied Medical のバルーントロカール特許群の**日本ファミリーと満了日**を Espacenet/INPADOC で確定、⒞ **CPC A61B17/34 の悉皆調査**、⒟ **J-PlatPat での国内出願悉皆（八光・オリンパス等）**、⒠ KR 10-2058617 の日米ファミリーの有無。本調査は J-PlatPat に到達できておらず**国内出願の悉皆性が未担保** |

### 次に着手すべきこと

| # | 内容 |
|---|---|
| 4 | **非OA の直接先行技術2件を入手**：Qi/Balkhy 2025（[PMID 40913323](https://pubmed.ncbi.nlm.nih.gov/40913323/)）＝カテーテル仕様・バルーン容量・**対照群6例の出血源の記載の有無**／Kiani 2012（[PMID 23092664](https://pubmed.ncbi.nlm.nih.gov/23092664/)）＝使用した流動性止血材の製品名と、**有意差が輸血率のみだったことの確認**。**全文を読まずに提案書を書かないこと** |
| 5 | **既存品の ad hoc 組合せで PoC を取る。** ①ポート抜去 → ②Foley を刺入路へ挿入・膨張 → ③止血材充填 → ④段階減圧・抜去、を数例で実施し手技記録・出血量・ドレーン量を取る。症例報告で論文化でき、かつ機能要件（バルーン容量・留置時間・減圧速度）が実測で決まる |
| 6 | **動物試験（ブタ肋間）でパラメータを決める。** バルーン形状（球状 vs 扁平ディスク状）× 膨張圧（定容 vs 定圧）× 留置時間。エンドポイントは**肋間神経の組織学（軸索変性）**・皮膚/肋間筋の圧迫壊死・除圧後の再出血・バルーン破裂圧。**肋間でのバルーン圧迫の神経障害を評価した文献は世界に存在しない**ため、このデータ自体が論文・特許の裏づけになる |
| 7 | **材料の絞り込み。** 要件は「**非酸性・非膨潤・乾燥プレロード可・非生物由来**」。**ユーザーが名指しした4材料（ニューニット/サージセル/サージフロー/タコシール）はいずれもこの要件を満たさない**（§3.3）ことを先に受け入れる。調査すべき候補〔いずれも要一次確認〕：⒜ 中和処理 ORC（Equitamp® pH4.8 / Equicel®）の日本承認状況、⒝ Hemopatch（NHS-PEG コーティングコラーゲン）、⒞ 形状記憶ポリマーフォーム／ハイドロゲル複合体（[PMID 38130883](https://pubmed.ncbi.nlm.nih.gov/38130883/)・[36729490](https://pubmed.ncbi.nlm.nih.gov/36729490/)・42026758）、⒟ **キトサン被覆デバイスの前例（止血尿道カテーテル、[PMID 35461919](https://pubmed.ncbi.nlm.nih.gov/35461919/)）— 表面担持止血材の擦れ落ち・溶出制御・留置中性能という実装課題を先取りしている可能性が高く全文精読推奨** |
| 8 | **ロボットアーム荷重の実測。** remote center のズレ量と肋間・皮膚に加わる力（N）・モーメントを、術中のアーム位置ログまたはポート部の力センサで取得。臨床・工学いずれにも先行データがなく、**設計入力（design input）を自前で作る必要がある** |

### 相談先

| # | 内容 |
|---|---|
| 9 | **PMDA 全般相談 → 該当性／区分の確認 → RS戦略相談。** 「止血材を装着したトロカールスリーブ」が**認証**（八光 E・ZバルーンⅡ と同じ「単回使用トロカールスリーブ」JMDN 37148002）で済むのか**承認**が必要か、生物由来材料を避ければコンビネーション扱いを回避できるかを早期に確定。**ここが開発費と期間を桁で変える** |
| 10 | **企業との接点。** ⒜ **八光**（既に同一コンセプトの国産バルーントロカールを持ち、共同開発の適性がある）、⒝ Applied Medical／オリンパス（Kii の国内販売元）へのライセンス／OEM 打診、⒞ **ConMed 型モデル**（Intuitive と共同で工学的・技術的互換性試験を実施し自社名義で 510(k)）の可否を Intuitive Japan に打診。**反トラスト訴訟は法的てこには使えないが、敵対する必要もない。協業モデルが現実解** |
| 11 | **アカデミア。** 神戸大（Tanaka/Maniwa＝ORC リング付き創縁保護材の RCT）・兵庫医大（Nakamura/Funaki＝hinotori トロカールレス）。**いずれも兵庫県内でメディカロイド（神戸）と同じ圏内** |
| 12 | **公的資金。** AMED 医工連携イノベーション推進事業、医療機器開発支援ネットワーク（伴走コンサル）。**#1 の記述研究を先に走らせておくと申請の説得力が桁違いになる** |

### 対外的に主張してはいけないこと（そのまま反証される）

- ✕「バルーンで止血する胸壁ポートは世界初」→ 八光 E・ZバルーンⅡ、Applied Medical Kii、[US 8,142,467](https://www.freepatentsonline.com/8142467.html)、Balkhy の臨床実装で潰れる
- ✕「ポート孔出血がロボット心臓手術の再開胸の主要原因である」→ ロボット 8mm ポートに限った出血源内訳データはゼロ。非ロボット MICS 3シリーズ計437例・再開胸10例中8例が胸壁系という粗い数字しかなく**点推定不能**
- ✕「Qi 2025 が概念実証を済ませている」→ 無調整の前後比較、説明不能なペースメーカー植込み率の差、COI（§2.3）
- ✕「電メがポート創を悪くする（確立した事実として）」→ 一般外科のメタ解析で即座に反証される。**Patel/Macoskey の専門家記述を引きつつ「比較研究が無いので我々が測る」と立てること**（§4.3）
