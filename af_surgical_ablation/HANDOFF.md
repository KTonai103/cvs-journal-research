...
>
> ## ✅ 追補2（2026-07-25、同日）— 略語の初出定義＋A–Z 略語集ポップアップ
> - **初出定義**: 本文（表・図・英文引用・コード以外）の初出位置に `略語（英語フルスペル：日本語）` を35箇所挿入。
>   スクリプトで自動挿入したが、**`（` の直後が数値の場合を「既に定義済み」と誤判定する**ので
>   （`CTI（28.6%）`・`LAVI（OR 1.05`・`CABG（10例）`）、必ず結果を目視すること。
>   **括弧の内側・英文引用の内側・`**…**` の内側には入れない**（ネストした `**` は pandoc の太字を壊す）。
>   統計略語（CI/HR/OR/RR/SHR/IQR…）は必ず数値と一緒に括弧内に出るため**インライン定義に向かず**、
>   冒頭に新設した `## 本稿の表記と略語`（病態・術式・判定／統計・研究デザインの2表）でまとめて定義した。
> - **略語集ポップアップ**: `glossary.json`（180項目、`a`=略語 / `en` / `ja` / `cat`）を `build_html.py` が
>   `<script type="application/json">` として埋め込み、右上固定ボタン（キーボード **`a`**）で開くモーダルを生成する。
>   A–Z チップ（該当なしの文字は disabled）＋絞り込み検索＋**行クリックでその略語を本文検索**
>   （既存の検索処理を `window.__afSearch` として公開して再利用）。`#gloss` でディープリンク可。
> - 検証: Playwright MCP が別セッションのプロファイルロックで使えない場合、
>   **ビルド済みHTMLに自己テスト用 `<script>` を注入したコピーを localhost に置き、
>   結果を固定 div に書き出して headless shell でスクリーンショットする**ことで
>   クリック・キーボード・フィルタまで検証できる（実績: 180行→「三尖」12行/「hazard」2行、
>   行クリックで `search-input="CTSN"`・21ハイライト、`a` 開閉・Esc・背景クリック・`/` の非干渉すべて確認）。
