#!/usr/bin/env python3
"""Build the search-enabled, sidebar-navigated AAE technique atlas HTML report.

Reuses the house CSS / callout / pandoc pipeline from convert_to_html.py and the
full-text search box + sidebar TOC from robotic_cpb/build_html.py (same as cor_knot).

Usage:
  python3 aae_techniques/build_html.py            # defaults to the AAE atlas
  python3 aae_techniques/build_html.py in.md out.html
"""
import os
import re
import sys
...
DESCRIPTION = ("大動脈弁輪拡大術（AAE）テクニカル・アトラス — Nicks / Manouguian / Konno / Y-incision の"
               "手技・拡大量・固有リスクを自作模式図とオープンアクセス術中図で総括し、日本人小体格に対する"
               "術式選択（aortomitral curtain 8mm の解剖学的天井・Y-incision後central MR・ViV戦略）を実践的に整理")
...
def main():
    if len(sys.argv) == 3:
        ...
        md = os.path.join(BASE, "md", "aae_technique_atlas.md")
        out = os.path.join(ROOT, "output", "aae_technique_atlas.html")
        build(md, out)
